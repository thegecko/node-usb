...
// const binding = bindings('usb_bindings.node');

import * as binding from '../build/Release/usb_bindings'


if (binding.INIT_ERROR) {
	console.warn("Failed to initialize libusb.")
	binding.Device = function () { throw new Error("Device cannot be instantiated directly.") };
	binding.Transfer = function () { throw new Error("Transfer cannot be instantiated directly.") };
	binding.setDebugLevel = function () { };
	binding.getDeviceList = function () { return []; };
	binding._enableHotplugEvents = function () { };
	binding._disableHotplugEvents = function () { };
}

const isBuffer = (obj: any): obj is Uint8Array => obj && obj instanceof Uint8Array;

class usb extends binding, EventEmitter {

}

const instance = new usb();
...
Object.keys(EventEmitter.prototype).forEach(function (key) {
	exports[key] = EventEmitter.prototype[key];
});

// convenience method for finding a device by vendor and product id
exports.findByIds = function(vid: number, pid: number) {
...
usb.Device.prototype.open = (defaultConfig: any) => {
	this.__open()
